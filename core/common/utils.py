...
def get_spelling_variant(text):
    """
    Return the US/British spelling alternative for a given text, or None if no
    substitution applies.  Covers the most common medical spelling divergences
    (haem/hem, paed/ped, -aemia/-emia, -oe-/-e-, -our/-or, -ise/-ize, -yse/-yze).
    Both directions are tried so either variant in the query finds the other in the
    index.
    """
    import re
    ...
    british_to_us = [
        (r'(?i)haemorrhag',  'hemorrhag'),
        (r'(?i)haematolog',  'hematolog'),
        (r'(?i)haematom',    'hematom'),
        (r'(?i)haemogl',     'hemogl'),
        (r'(?i)haemophil',   'hemophil'),
        (r'(?i)haem(?!orrhag|atolog|atom|ogl|ophil)', 'hem'),
        ...
        (r'(?i)oedema',      'edema'),
        (r'(?i)oesophag',    'esophag'),
        (r'(?i)oestrogen',   'estrogen'),
        (r'(?i)orthopaed',   'orthoped'),
        (r'(?i)foetus',      'fetus'),
        (r'(?i)foetal',      'fetal'),
        (r'(?i)diarrhoea',   'diarrhea'),
        (r'(?i)homoeopat',   'homeopat'),
        (r'(?i)colour',      'color'),
        (r'(?i)tumour',      'tumor'),
        (r'(?i)behaviour',   'behavior'),
        (r'(?i)flavour',     'flavor'),
        (r'(?i)honour',      'honor'),
        (r'(?i)neighbour',   'neighbor'),
        (r'(?i)analyse',     'analyze'),
        (r'(?i)paralyse',    'paralyze'),
        (r'(?i)catalyse',    'catalyze'),
        (r'(?i)hydrolyse',   'hydrolyze'),
        (r'(?i)specialise',  'specialize'),
        (r'(?i)recognise',   'recognize'),
        (r'(?i)organise',    'organize'),
        (r'(?i)mobilise',    'mobilize'),
        (r'(?i)stabilise',   'stabilize'),
        (r'(?i)normalise',   'normalize'),
        (r'(?i)localise',    'localize'),
    ]

    us_to_british = [
        (r'(?i)hemorrhag',   'haemorrhag'),
        (r'(?i)hematolog',   'haematolog'),
        (r'(?i)hematom',     'haematom'),
        (r'(?i)hemogl',      'haemogl'),
        (r'(?i)hemophil',    'haemophil'),
        (r'(?i)hem(?!orrhag|atolog|atom|ogl|ophil)', 'haem'),
        (r'(?i)leukemia',    'leukaemia'),
        (r'(?i)anemia',      'anaemia'),
        (r'(?i)pediatric',   'paediatric'),
        (r'(?i)pediat(?!ric)', 'paediat'),
        (r'(?i)gynecolog',   'gynaecolog'),
        (r'(?i)edema',       'oedema'),
        (r'(?i)esophag',     'oesophag'),
        (r'(?i)estrogen',    'oestrogen'),
        (r'(?i)orthoped',    'orthopaed'),
        (r'(?i)fetus',       'foetus'),
        (r'(?i)fetal',       'foetal'),
        (r'(?i)diarrhea',    'diarrhoea'),
        (r'(?i)homeopat',    'homoeopat'),
        (r'(?i)\bcolor\b',   'colour'),
        (r'(?i)\btumor\b',   'tumour'),
        (r'(?i)\bbehavior\b', 'behaviour'),
        (r'(?i)\bflavor\b',  'flavour'),
        (r'(?i)\bhonor\b',   'honour'),
        (r'(?i)\bneighbor\b', 'neighbour'),
        (r'(?i)analyze',     'analyse'),
        (r'(?i)paralyze',    'paralyse'),
        (r'(?i)catalyze',    'catalyse'),
        (r'(?i)hydrolyze',   'hydrolyse'),
        (r'(?i)specialize',  'specialise'),
        (r'(?i)recognize',   'recognise'),
        (r'(?i)organize',    'organise'),
        (r'(?i)mobilize',    'mobilise'),
        (r'(?i)stabilize',   'stabilise'),
        (r'(?i)normalize',   'normalise'),
        (r'(?i)localize',    'localise'),
    ]

    for pattern, replacement in british_to_us:
        substituted = re.sub(pattern, replacement, text)
        if substituted != text:
            return substituted

    for pattern, replacement in us_to_british:
        substituted = re.sub(pattern, replacement, text)
        if substituted != text:
            return substituted
