...
def resolve_description(concept: Concept) -> Optional[str]:
    cached = getattr(concept, '_prefetched_objects_cache', {})
    descriptions = (
        [description for description in cached['descriptions'] if not description.retired]
        if 'descriptions' in cached else list(concept.active_descriptions.all())
    )
    if not descriptions:
        return None
...
def serialize_concepts(concepts: Iterable[Concept], paths=None) -> List[ConceptType]:
    def requested(field):
        """Detect selected scalar or nested values without evaluating model properties."""
        return paths is None or any(path == field or path.startswith(field + '.') for path in paths)
    ...
    for concept in concepts:
        output.append(
            ConceptType(
                id=str(concept.id),
                external_id=concept.external_id if requested('externalId') else None,
                concept_id=concept.mnemonic,
                display=concept.display_name if requested('display') else None,
                names=serialize_names(concept) if requested('names') else [],
                mappings=serialize_mappings(concept) if requested('mappings') else [],
                description=resolve_description(concept) if requested('description') else None,
                concept_class=concept.concept_class if requested('conceptClass') else None,
                datatype=(
                    build_datatype(concept) if paths is None or requested('datatype.details') else
                    DatatypeType(name=concept.datatype, details=None) if concept.datatype else None
                ) if requested('datatype') else None,
                metadata=build_metadata(concept) if requested('metadata') else None,
                extras=(concept.extras or {}) if requested('extras') else {},
            )
        )
    return output
