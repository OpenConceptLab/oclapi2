...
bash wait_for_it.sh ${DB_HOST}:${DB_PORT} -t 0

if [[ -z "$ES_HOSTS" ]]; then
bash wait_for_it.sh ${ES_HOST}:${ES_PORT} -t 0
else
set +e
ES_HOSTS_LIST=(${ES_HOSTS//,/ })
for ES in "${ES_HOSTS_LIST[@]}"; do
  bash wait_for_it.sh ${ES} -t 180
done
set -e
fi

bash wait_for_it.sh ${REDIS_HOST}:${REDIS_PORT} -t 0
